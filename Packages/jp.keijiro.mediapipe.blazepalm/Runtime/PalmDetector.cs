...
public sealed partial class PalmDetector : System.IDisposable
{
    ...
    void AllocateObjects(ResourceSet resources)
    {
        ...
        _size = model.inputs[0].shape.ToTensorShape()[3];
    ...
    void RunModel(float threshold)
    {
        ...
        var scoresBuffer = ((ComputeTensorData)(_worker.PeekOutput("classificators") as Tensor<float>).dataOnBackend).buffer;
        var boxesBuffer  = ((ComputeTensorData)(_worker.PeekOutput("regressors")     as Tensor<float>).dataOnBackend).buffer;

        post1.SetBuffer(0, "_Scores", scoresBuffer);
        post1.SetBuffer(0, "_Boxes",  boxesBuffer);
        post1.SetBuffer(0, "_Output", _output.post1);
        post1.Dispatch(0, 1, 1, 1);

        post1.SetBuffer(1, "_Scores", scoresBuffer);
        post1.SetBuffer(1, "_Boxes",  boxesBuffer);
